    ...
    <header className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 backdrop-blur-md">
      <div className="w-full px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          {/* Student Profile Quick Switcher (when in Student role) */}
          {activeRole === 'student' && students.length > 0 && onSwitchStudent && (
            <div className="relative">
              <button
                id="nav-student-selector-btn"
                type="button"
                onClick={() => setShowStudentDropdown(prev => !prev)}
                className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-indigo-200/80 bg-indigo-50/70 hover:bg-indigo-100/70 text-indigo-900 text-xs font-medium transition-colors shadow-xs"
              ...
              >
                ...
                <ChevronDown className="w-3 h-3 text-indigo-500 opacity-70" />
              </button>

              {showStudentDropdown && (
                <div
                  id="nav-student-dropdown-menu"
                  className="absolute right-0 mt-1.5 w-72 max-h-96 overflow-y-auto rounded-xl bg-white border border-slate-200 shadow-xl py-1.5 z-50 divide-y divide-slate-100"
                >
                  <div className="px-3 py-2 bg-slate-50">
                    <p className="text-[11px] font-bold text-slate-700 uppercase tracking-wider">
                      Switch Student Profile ({students.length} Unique Cohorts)
                    </p>
                    <p className="text-[10px] text-slate-500">
                      Each student has verified unique skills, scores & projects
                    </p>
                  </div>
                  <div className="py-1">
                    {students.map((s) => {
                        ...
                        >
                          <div className="min-w-0 pr-2">
                            ...
                            <div className="text-[11px] text-slate-500 truncate">
                              {s.careerGoal || 'AI Engineer'} • {s.college?.split(' ')[0] || 'College'}
                            </div>
                          </div>
                          <div className="text-right shrink-0">
                            <span className="text-[11px] font-bold text-indigo-600">
                              {s.careerReadinessScore || 75}%
                            </span>
